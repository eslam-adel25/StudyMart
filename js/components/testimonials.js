...
export function renderHomeTestimonials() {
  ...
  const existingViewAll = reviewsSection.querySelector(
    ".view-all-reviews-wrapper",
  );
...
function getReviewSearchableText(review) {
  ...
  if (review.type === "Course" && review.purchasedItemId) {
    const course = coursesData.find(
      (c) => String(c.id) === String(review.purchasedItemId),
    );
    if (course) {
    ...
    const book = booksData.find(
      (b) => String(b.id) === String(review.purchasedItemId),
    );
    if (book) {
...
function getFilteredPublicReviews() {
  ...
  if (publicReviewState.starsFilter !== "all") {
    filtered = filtered.filter(
      (r) => String(r.stars) === String(publicReviewState.starsFilter),
    );
  }
...
export function renderPublicReviewsListOnly() {
  ...
  const paginationEl = container.querySelector(
    ".public-reviews-pagination-wrapper",
  );
  ...
  const totalPages =
    Math.ceil(filtered.length / publicReviewState.pageSize) || 1;
  if (publicReviewState.currentPage > totalPages)
    publicReviewState.currentPage = 1;

  const startIndex =
    (publicReviewState.currentPage - 1) * publicReviewState.pageSize;
  const paginated = filtered.slice(
    startIndex,
    startIndex + publicReviewState.pageSize,
  );
...
export function renderPublicReviewsUI() {
  ...
  const avgRating = totalCount
    ? (
        allReviews.reduce((sum, r) => sum + (Number(r.stars) || 5), 0) /
        totalCount
      ).toFixed(1)
    : "0.0";
