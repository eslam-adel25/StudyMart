...
export async function buildInvoiceHTMLString(tx) {
  ...
  const genTime = new Date().toLocaleString("ar-EG", {
    year: "numeric",
    month: "short",
    day: "numeric",
    hour: "2-digit",
  ...
  const buyerEmail =
    tx.studentEmail || tx.buyerEmail || "student@studymart.com";
  ...
  const buyerAddress =
    tx.billingAddress || tx.buyerAddress || "شارع العليا العام - المجمع المالي";
  const buyerAccountId =
    tx.accountID ||
    tx.buyerAccountId ||
    `STD-${Math.abs(String(txId).replace(/\D/g, "") || "9012")}`;

  const teacherName = tx.teacherName || tx.sellerName || "د. محمد أحمد العتيبي";
  const teacherEmail =
    tx.teacherEmail || tx.sellerEmail || "teacher@studymart.com";
  ...
  const platformFeeVal =
    tx.platformFee !== undefined
      ? tx.platformFee
      : Math.round(totalOriginalPrice * 0.1 * 100) / 100;
  const platformFeeStr = platformFeeVal.toFixed(2);

  const netRevenueVal =
    tx.netRevenue !== undefined
      ? tx.netRevenue
      : Math.max(0, totalFinalPaid - platformFeeVal - totalTaxVal);
  const netRevenueStr = netRevenueVal.toFixed(2);

  // Render Table Rows
  const tableRowsHTML = itemsArray
    .map((item, idx) => {
      const itemTitle = item.title || item.bookName || item.name || "منتج رقمي";
      const isCourseItem =
        item.productType === "Course" || item.type === "course";
      const isBookItem = item.productType === "Book" || item.type === "book";
      const typeLabel = isCourseItem
        ? "🎓 دورة تدريبية"
        : isBookItem
          ? "📚 كتاب رقمي"
          : item.productType || "منتج رقمي";
      ...
      const itemTotal = (
        ...
        Math.max(
          0,
          (item.price || item.originalPrice || 0) - (item.discount || 0),
        )
      ).toFixed(2);
...
export async function openInvoiceModal(txOrId) {
  ...
  if (typeof txOrId === "object" && txOrId !== null) {
    ...
    tx = transactionsData.find(
      (t) =>
        t.id === String(txOrId) ||
        t.id === `TXN-${txOrId}` ||
        t.invoiceNo === String(txOrId),
    );
...
export async function downloadInvoicePDFDocument(tx) {
  try {
    ...
    showCustomAlert(
      "حدث خطأ أثناء إنشاء ملف PDF. جاري تحويلك للطباعة المباشرة...",
    );
    printInvoiceDocument(tx);
  }
}
 ...
 */
export async function exportTransactionsPDFReport(
  filteredList,
  filterSummary,
  teacherName = "د. محمد أحمد العتيبي",
) {
  ...
  try {
    ...
    const completedOrders = filteredList.filter(
      (t) => t.status === "Completed",
    ).length;
    const pendingOrders = filteredList.filter(
      (t) => t.status === "Pending",
    ).length;
    const cancelledOrders = filteredList.filter(
      (t) => t.status === "Cancelled",
    ).length;
    const refundCount = filteredList.filter(
      (t) => t.type === "Refund" || t.status === "Refunded",
    ).length;

    const coursesSold = filteredList.filter(
      (t) => t.productType === "Course" && t.type === "Purchase",
    ).length;
    const booksSold = filteredList.filter(
      (t) =>
        (t.productType === "Book" || !t.productType) && t.type === "Purchase",
    ).length;

    const grossRevenue = filteredList
      .filter((t) => t.type === "Purchase" && t.status === "Completed")
      .reduce((sum, t) => sum + (t.price || 0), 0);
    const discountTotal = filteredList.reduce(
      (sum, t) => sum + (t.discount || 0),
      0,
    );
    const taxTotal = filteredList.reduce((sum, t) => sum + (t.tax || 0), 0);
    const platformRevenue = filteredList.reduce(
      (sum, t) => sum + (t.platformFee || 0),
      0,
    );
    const teacherRevenue = filteredList
    ...
    const reportElem =
      tempDiv.querySelector("#transactionsReportPrintArea") || tempDiv;
    ...
    showCustomAlert(
      "حدث خطأ أثناء تصدير التقرير إلى PDF. يرجى المحاولة مرة أخرى.",
    );
  }
}
